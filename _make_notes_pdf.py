...
try:
    ...
    from reportlab.lib.pagesizes import A4
    from reportlab.pdfgen import canvas
root = Path(r'C:\wrhor\DataBase')
files = [
    (root / 'RELEASE_NOTES.md', root / 'RELEASE_NOTES.pdf'),
    (root / 'RELEASE_NOTES_CLIENT.md', root / 'RELEASE_NOTES_CLIENT.pdf'),
    (root / 'how_to.md', root / 'app' / 'src' / 'main' / 'assets' / 'how_to.pdf'),
]
PAGE_W, PAGE_H = A4
...
for src, dst in files:
    ...
    for raw in lines:
        ...
        if not line:
            y -= 15
            if y < BOTTOM:
                c.showPage(); y = PAGE_H - TOP
            continue
        text = line
        font = 'Helvetica'
        size = 11
        if text.startswith('# '):
            text = text[2:].strip(); font = 'Helvetica-Bold'; size = 16
        elif text.startswith('## '):
            text = text[3:].strip(); font = 'Helvetica-Bold'; size = 13
        ...
        for part in wrapped:
            if y < BOTTOM:
                c.showPage(); y = PAGE_H - TOP; c.setFont(font, size)
